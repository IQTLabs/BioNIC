...
4. Use VennData to make measurements about the quality of our datasets, and see if we can get better results with that knowledge
...
# #########################################################################################################################################################
batch_size = 8
epochs = 15             # default epochs (can be overriden in model config)
learning_rate = 1e-4
device = "cuda"
...
drop_last = True        # BatchNorm has issues if the samples aren't in the min_size of the batch, or the batch has one sample in it
...
if calcStats == False:
    ...
    # you will set this to True the first time you run this code on a new dataset to generate a global holdout, then set it to False
...
def runTrainAndTest(train_dataset, test_dataset, dataloaders, f, results_normal):
    ...
    neurons.train(dataloaders['train'], None, epochs, device, model, f)
...
if model['usesCV']:
    ...
    #holdout_normal = ImageFolderWithPaths(root=SINGLE_IMAGE, transform=image_transforms['test'])   # if we want to evaluate the trained model 
                                                                                                    # on just a single image, for blog post purposes
